...
#Pindahkan informasi konfigurasi versi lama untuk menyesuaikan dengan versi kurang dari 1.1.0
[[ -f "/etc/v2ray/vmess_qr.json" ]] && mv /etc/v2ray/vmess_qr.json $v2ray_qr_config_file

#Nomor acak sederhana
random_num=$((RANDOM%12+4))
#Hasilkan jalur kamuflase
...
#Ekstrak nama bahasa Inggris dari sistem distribusi dari VERSION, untuk menambahkan sumber apt Nginx yang sesuai di bawah debian / ubuntu
VERSION=$(echo "${VERSION}" | awk -F "[()]" '{print $2}')

check_system() {
    if [[ "${ID}" == "centos" && ${VERSION_ID} -ge 7 ]]; then
        echo -e "${OK} ${GreenBG} OS Vps Anda Centos ${VERSION_ID} ${VERSION} ${Font}"
        INS="yum"
    elif [[ "${ID}" == "debian" && ${VERSION_ID} -ge 8 ]]; then
        echo -e "${OK} ${GreenBG} OS Vps Anda Debian ${VERSION_ID} ${VERSION} ${Font}"
        INS="apt"
        $INS update
        ## Tambahkan sumber Nginx apt
    elif [[ "${ID}" == "ubuntu" && $(echo "${VERSION_ID}" | cut -d '.' -f1) -ge 16 ]]; then
        echo -e "${OK} ${GreenBG} OS Vps Anda Ubuntu ${VERSION_ID} ${UBUNTU_CODENAME} ${Font}"
        INS="apt"
        ...
        echo -e "${Error} ${RedBG} OS anda adalah ${ID} ${VERSION_ID} OS anda tidak terdaftar, penginstallan terhenti ${Font}"
        exit 1
    fi
    ...
    echo -e "${OK} ${GreenBG} firewalld done ${Font}"

    systemctl stop ufw
    systemctl disable ufw
    echo -e "${OK} ${GreenBG} ufw done ${Font}"
}

is_root() {
    if [ 0 == $UID ]; then
        echo -e "${OK} ${GreenBG} Pengguna saat ini adalah pengguna root, masukkan proses instalasi ${Font}"
        sleep 3
    else
        echo -e "${Error} ${RedBG} Pengguna saat ini bukanlah pengguna root, harap beralih ke pengguna root dan jalankan kembali skrip ${Font}"
        exit 1
    fi
}
judge() {
    if [[ 0 -eq $? ]]; then
        echo -e "${OK} ${GreenBG} $1 sukses ${Font}"
        sleep 1
    else
        echo -e "${Error} ${RedBG} $1 gagal ${Font}"
        exit 1
    fi
}
chrony_install() {
    ${INS} -y install chrony
    judge "Instal layanan sinkronisasi waktu kronik "
    ...
    echo -e "${OK} ${GreenBG} Menunggu sinkronisasi..... ${Font}"
    sleep 10

    chronyc sourcestats -v
    chronyc tracking -v
    date
    read -rp "Klik Y untuk melanjutkan (Y/N): " chrony_install
    [[ -z ${chrony_install} ]] && chrony_install="Y"
    case $chrony_install in
    [yY][eE][sS] | [yY])
        echo -e "${GreenBG} Lanjutkan menginstal ${Font}"
        sleep 2
        ;;
    *)
        echo -e "${RedBG} Instalasi dihentikan ${Font}"
        exit 2
        ;;
    esac
}
...
port_alterid_set() {
    if [[ "on" != "$old_config_status" ]]; then
        read -rp "Input port（default:443）:" port
        [[ -z ${port} ]] && port="443"
        read -rp "Input alterID（default:2）:" alterID
        [[ -z ${alterID} ]] && alterID="2"
    fi
}
modify_path() {
...
v2ray_install() {
    ...
    if [[ -f v2ray.sh ]]; then
        ...
        bash v2ray.sh --force
        judge "instalasi V2ray"
    else
        echo -e "${Error} ${RedBG} Install v2ray gagal, cek website anda ${Font}"
        exit 4
    fi
    # 清除临时文件
    rm -rf /root/v2ray
...
nginx_exist_check() {
    if [[ -f "/etc/nginx/sbin/nginx" ]]; then
        ...
        echo -e "${OK} ${GreenBG} Nginx yang diinstal oleh paket lain terdeteksi. Jika Anda terus menginstalnya, itu akan menyebabkan konflik. Silakan instal setelah menangani. ${Font}"
        exit 1
    else
        nginx_install
    fi
...
nginx_install() {
    ...
    echo -e "${OK} ${GreenBG} Install ngnix, wait for 5 minutes.. ${Font}"
    sleep 4
...
ssl_install() {
    ...
    judge "pembuatan sertifikat SSL"

    curl https://get.acme.sh | sh
    judge "安装 SSL 证书生成脚本"
}
domain_check() {
    read -rp "Input domain :(eg:www.xanstech.com):" domain
    domain_ip=$(ping "${domain}" -c 1 | sed '1{s/[^(]*(//;s/).*//;q}')
    echo -e "${OK} ${GreenBG} IP public terdeteksi, tunggu sebentar... ${Font}"
    local_ip=$(curl https://api-ipv4.ip.sb/ip)
    echo -e "DNS Domain IP：${domain_ip}"
    ...
    if [[ $(echo "${local_ip}" | tr '.' '+' | bc) -eq $(echo "${domain_ip}" | tr '.' '+' | bc) ]]; then
        ...
        echo -e "${Error} ${RedBG} IP resolusi DNS dari nama domain tidak cocok dengan IP lokal. Apakah Anda ingin melanjutkan penginstalan?（y/n）${Font}" && read -r install
        case $install in
        [yY][eE][sS] | [yY])
            echo -e "${GreenBG}Lanjutkan menginstal  ${Font}"
            sleep 2
...
port_exist_check() {
    if [[ 0 -eq $(lsof -i:"$1" | grep -i -c "listen") ]]; then
        echo -e "${OK} ${GreenBG} $1 Port kosong ${Font}"
        sleep 1
    else
        echo -e "${Error} ${RedBG} terdeteksi $1 Port sudah ditempati, berikut ini $1 Info port ${Font}"
        lsof -i:"$1"
...
acme() {
    if "$HOME"/.acme.sh/acme.sh --issue -d "${domain}" --standalone -k ec-256 --force --test; then
        echo -e "${OK} ${GreenBG} Penerbitan uji sertifikat SSL berhasil, dan penerbitan resmi dimulai. ${Font}"
        rm -rf "$HOME/.acme.sh/${domain}_ecc"
        sleep 2
    else
        echo -e "${Error} ${RedBG} Penerbitan uji sertifikat SSL gagal ${Font}"
        rm -rf "$HOME/.acme.sh/${domain}_ecc"
        exit 1
    fi

    if "$HOME"/.acme.sh/acme.sh --issue -d "${domain}" --standalone -k ec-256 --force; then
        echo -e "${OK} ${GreenBG} Sertifikat SSL berhasil dibuat ${Font}"
        sleep 2
        mkdir /data
        ...
        echo -e "${Error} ${RedBG} Pembuatan sertifikat SSL gagal ${Font}"
        rm -rf "$HOME/.acme.sh/${domain}_ecc"
        exit 1
    fi
}
...
old_config_exist_check() {
    if [[ -f $v2ray_qr_config_file ]]; then
        echo -e "${OK} ${GreenBG} File konfigurasi lama terdeteksi, apakah ingin menggunakan konfigurasi file lama [Y/N]? ${Font}"
        read -r ssl_delete
        case $ssl_delete in
        [yY][eE][sS] | [yY])
            echo -e "${OK} ${GreenBG} Konfigurasi lama telah dipertahankan  ${Font}"
...
vmess_qr_link_image() {
    vmess_link="vmess://$(base64 -w 0 $v2ray_qr_config_file)"
    {
        ...
        echo -e "${Red} URL :${vmess_link} ${Font}"
    } >>"${v2ray_info_file}"
}
...
vmess_link_image_choice() {
        echo "Pilih jenis URL yang dibuat"
        echo "1: V2RayNG/V2RayN"
        echo "2: quantumult"
        read -rp "Input：" link_version
...
basic_information() {
    {
        echo -e "${OK} ${GreenBG} V2ray+ws+tls Created succes"
        echo -e "${Red} V2ray info ${Font}"
        ...
        echo -e "${Red} Alter ID（alterId）：${Font} $(info_extraction '\"aid\"')"
        echo -e "${Red} Security（security）：${Font} Adaptive "
        echo -e "${Red} Network type（network）：${Font} $(info_extraction '\"net\"') "
        echo -e "${Red} Header type （type）：${Font} none "
        echo -e "${Red} Path/Seed ：${Font} $(info_extraction '\"path\"') "
        echo -e "${Red} Transmission security：${Font} tls "
    } >"${v2ray_info_file}"
}
show_information() {
    cat "${v2ray_info_file}"
...
tls_type() {
    if [[ -f "/etc/nginx/sbin/nginx" ]] && [[ -f "$nginx_conf" ]] && [[ "$shell_mode" == "ws" ]]; then
        ...
        echo -e "${Error} ${RedBG} Nginx or configuration file does not exist or the currently installed version is h2, please install the script correctly and execute it ${Font}"
    fi
}
show_access_log() {
...
update_sh() {
    ...
    if [[ "$shell_version" < "$(sort -rV $version_cmp | head -1)" ]]; then
        echo -e "${OK} ${GreenBG}Ada versi baru, apakah akan memperbarui [Y/N]? ${Font}"
